 ...
 *
 */
class CompositeEntity: public Element {
protected:
...
public:
    ...
    T* spawn_entity(Args &&...args) {
        ...
        if constexpr (std::is_base_of<Element, T>()) {

            // T is an Element -> create an instance and push it
     ...
     * @param child
     */
    void delete_entity(Element* ent);
    ...
    virtual void on_tick(const float& delta) override;

    virtual void on_lerp(float t) override;
